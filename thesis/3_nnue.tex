...
The objective is to take advantage of modern CPUs that allow doing low-precision integer arithmetic in parallel with 8, 16, 32 or even 64 8-bit integer values at a time. To take advantage of this, we want to use the smallest integer type possible everywhere, to process more values at once. \\
...
\paragraph[short]{Input} Inputs are quantized to 8 bits, so the range of values is $-128..127$. Since the inputs are hot encoded, the float values are 0.0 or 1.0, so the quantized values are either 0 or 127.
...
\paragraph[short]{Linear layer} asdasdasd

\paragraph[short]{ClippedReLU} asdasdasd
...
The implementation was tested comparing with the output of the Pytorch model.
